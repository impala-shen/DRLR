...
class Col_e(Agent):
    ...
    def init(self, trainer_cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        # create tensors in memory #todo should I initialize expert memory
        # # self.expert_memory
        if self.expert_memory is not None:
            ...
            self._tensors_names = ["states", "actions", "rewards", "next_states", "terminated"]
            # expert_memory = postprocessing.MemoryFileIterator("/home/chen/Downloads/new/memories/expert_new.pt")
            # for j, data in expert_memory:
            #     # self.expert_memory.add_samples(d)
            #     keys = list(data.keys())
            #     for i in range(0, 5000):
            #         self.expert_memory.add_samples(states=data[keys[3]][i],
            #                                        actions=data[keys[0]][i],
            #                                        rewards=data[keys[2]][i],
            #                                        next_states=data[keys[1]][i],
            #                                        terminated=data[keys[4]][i])
    ...
    def record_transition(self,
        ...
        if self.memory is not None:
            ...
            self.expert_memory.add_samples(states=states, actions=actions, rewards=rewards, next_states=next_states,
                                           terminated=terminated, truncated=truncated, **rnn_states)
            self.expert_memory.save("/home/chen/Downloads/new", "pt")
    ...
    def _update(self, timestep: int, timesteps: int, pre_train=False, experience_buffer_ratio=0.75, expert_buffer_ratio=0.25,
                LQ1Gain=1, LBCGain=1, LAGain=1) -> None:
        ...
        if pre_train:
            sampled_states, sampled_actions, sampled_rewards, sampled_next_states, sampled_dones = \
                self.expert_memory.sample(names=self._tensors_names, batch_size=self._batch_size,
                                   sequence_length=self._rnn_sequence_length)[0]
        else:
            sampled_states_r, sampled_actions_r, sampled_rewards_r, sampled_next_states_r, sampled_dones_r = \
                self.memory.sample(names=self._tensors_names, batch_size=int(self._batch_size * experience_buffer_ratio),
                                   sequence_length=self._rnn_sequence_length)[0]
            expert_states, expert_actions, expert_rewards, expert_next_states, expert_dones = \
                self.expert_memory.sample(names=self._tensors_names, batch_size=int(self._batch_size * expert_buffer_ratio),
                                   sequence_length=self._rnn_sequence_length)[0]
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            if not self._critic_update_counter % self._policy_delay:
                ...
                if pre_train:
                    bc_loss = LBCGain * F.mse_loss(actions, sampled_actions)
                    policy_loss = actor_loss * LAGain + bc_loss
                    # if self.total_it % 1000 == 0:
                    #     print(bc_loss, policy_loss, self.total_it)
                else:
                    actions, _, _ = self.policy.act({"states": expert_states, **rnn_policy}, role="policy")
                    policy_loss = actor_loss * LAGain + LBCGain * F.mse_loss(actions, expert_actions)
