...
class RFT(Agent):
    def __init__(self,
        ...
        """RFT
        :param models_il: NN used for Imitation learning in RFT paper, It is MLPs.
        :type models: dictionary of skrl.models.torch.Model
        :param models: NNs used by the RL agent, in RFT paper, they are all MLPs.
        :type models: dictionary of skrl.models.torch.Model
        ...
        """
        _cfg = copy.deepcopy(RFT_DEFAULT_CONFIG)
        ...
        # models
        self.policy = self.models_il.get("policy", None)
        ...
        if self.target_policy is not None and self.target_critic_1 is not None and self.target_critic_2 is not None:
            ...
            self.target_policy.update_parameters(self.IL_policy, polyak=1)
    ...
    def init(self, trainer_cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.memory is not None:
            ...
            # exp_memory = postprocessing.MemoryFileIterator("/home/chen/Downloads/new/memories/expert_cab_sparse.csv")
        ...
        if self.expert_memory is not None:
            ...
            # expert_memory = postprocessing.MemoryFileIterator("/home/chen/Downloads/new/memories/cartpole_exp.csv")    ## Others tasks
    ...
    def _update(self, timestep: int, timesteps: int, pre_train=False, soft = False, dynamics_bc_loss = True, experience_buffer_ratio=0.5, expert_buffer_ratio=1) -> None:
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            with torch.no_grad():
                ...
                discount_values = self._discount_factor * sampled_dones.logical_not() * target_q_values
            ...
            # critic_loss = F.mse_loss(critic_1_values, target_values) + F.mse_loss(critic_2_values, target_values)     ##oringinal critic update with 1-step Q loss
            ...
            if not self._critic_update_counter % self._policy_delay:
                ...
                # if pre_train:
                critic_values1, _, _ = self.critic_1.act({"states": sampled_states, "taken_actions": actions}, role="critic_1")
                critic_values2, _, _ = self.critic_2.act({"states": sampled_states, "taken_actions": actions}, role="critic_2")
                critic_values = torch.min(critic_values1, critic_values2)
                ...
                Lq1_loss = F.mse_loss(critic_values, target_values).mean()

                if pre_train:
                    bc_loss = F.mse_loss(actions, sampled_actions)
                    policy_loss = actor_loss + 1 * bc_loss.mean() + Lq1_loss*0
                else:
                    ...
                    ref_q, _, _ = self.critic_1.act({"states": sampled_states, "taken_actions": ref_actions},
                                                            role="critic_1")
                    ratio = (ref_q > critic_values).float().mean().item()
                    # policy_loss = (-critic_values*(1-ratio)).mean() + (10 * ratio * bc_loss).mean()
                    policy_loss = actor_loss + 10* bc_loss.mean() + Lq1_loss
                ...
                if pre_train:
                    ...
                    # self.IL_policy = copy.deepcopy(self.policy)
            ...
            if not self._critic_update_counter % self._policy_delay:
                ...
                self.track_data("Loss / 1-step q loss", torch.mean(Lq1_loss).item())
