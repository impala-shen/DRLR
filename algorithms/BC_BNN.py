...
BC_DEFAULT_CONFIG = {
    ...
    "exploration": {
        ...
        "initial_scale": 0.1,       # initial scale for the noise
...
class BC(Agent):
    ...
    def init(self, trainer_cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.expert_memory is not None:
            ...
            # expert_memory = postprocessing.MemoryFileIterator("/home/chen/Downloads/new/memories/expert_scale.csv")
    ...
    def act(self, states: torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        # BNN
        actions =self.policy(self._state_preprocessor(states))[0]
        # print(actions)

        # # Sample expert buffer
        # sampled_states, sampled_actions, sampled_rewards, sampled_next_states, sampled_dones = \
        #     self.expert_memory.sample(names=self._tensors_names, batch_size=1000)[0]
        #
        # _, uncertainty = self.policy.predict_with_uncertainty(self._state_preprocessor(sampled_states))
        ...
        # # Sample top states
        # sampled_states, sampled_actions, sampled_rewards, sampled_next_states, sampled_dones = \
        #     sampled_states[top_sample_indices], sampled_actions[top_sample_indices], sampled_rewards[top_sample_indices], sampled_next_states[top_sample_indices], sampled_dones[top_sample_indices]
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            # Compute actor loss
            actions = self.policy(sampled_states)[0]
            bc_loss = F.mse_loss(actions, sampled_actions)
            # KL loss
            kl_loss = self.policy.kl_divergence() / 10000
            ...
            self.track_data("Loss / Policy loss", loss.item())
            self.track_data("Loss / Critic loss", bc_loss.item())
