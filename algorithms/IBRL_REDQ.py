...
# from aem_BC import expert_memory
...
IBRL_DEFAULT_CONFIG = {
    ...
    "exploration": {
        ...
        "initial_scale": 1,       # initial scale for the noise
...
class IBRL(Agent):
    ...
    def init(self, trainer_cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.expert_memory is not None:
            ...
            self._tensors_names = ["states", "actions", "rewards", "next_states", "terminated"]
            # expert_memory = postprocessing.MemoryFileIterator("/home/chen/Downloads/new/memories/expert_p.csv")    ## AEM task
            # expert_memory = postprocessing.MemoryFileIterator("/home/chen/Downloads/new/memories/ad2.csv")    ## AEM task
        ...
        expert_states_r = self.expert_memory.sample(names=["states"],
                                      batch_size=N)[0][0]
    ...
    def _select_act(self, obs: torch.Tensor,
        exp_obs: torch.Tensor,
        soft: bool,
        target: bool):
        if target:
            # target policy smoothing
            rl_actions, _, _ = self.target_policy.act({"states": obs}, role="target_policy")

            if self._active:
                _, uncertainty = self.IL_policy.predict_with_uncertainty(self._state_preprocessor(obs))
                # noises = uncertainty*torch.sign(self._exploration_noise.sample(actions.shape))
                noises = uncertainty*self._exploration_noise.sample(rl_actions.shape)*10
                noises.clamp_(min = -self._smooth_regularization_clip, max = self._smooth_regularization_clip)
            else:
                # sample noises
                noises = torch.clamp(self._smooth_regularization_noise.sample(rl_actions.shape),
            ...
            if self._backward_compatibility:
                rl_actions = torch.max(torch.min(rl_actions, self.clip_actions_max), self.clip_actions_min)
            else:
        ...
        self.IL_policy.eval()
        if self._active:
            il_actions = self.IL_policy(self._state_preprocessor(exp_obs))
        else:
            il_actions, _, _ = self.IL_policy.act({"states": self._state_preprocessor(exp_obs)}, role="policy")

        # Stack actions and get batch dimensions
        rl_bc_actions = torch.stack([rl_actions, il_actions], dim=1)
        batch_size, num_action, _ = rl_bc_actions.size()  # get dimensions values, bsize:batch size
        ...
        if  torch.mean(target_q_il) > torch.mean(target_q_rl):
            if self._active:
                il_actions = self.IL_policy(self._state_preprocessor(obs))
            else:
    ...
    def act(self, states: torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        if timestep < self._random_timesteps:
            ...
            if self._active:
                actions = self.IL_policy(self._state_preprocessor(states))
                return [actions]
            else:
        ...
        if self._random_timesteps < timestep < self._learning_starts+self._random_timesteps:
            actions, _, _ = self.IL_policy.act({"states": self._state_preprocessor(states)}, role="policy")
        ...
        expert_states_r, expert_actions_r, expert_rewards_r, expert_next_states_r, expert_dones_r = \
            self.expert_memory.sample(names=self._tensors_names,
                                      batch_size=self._num_envs)[0]

        # noises = torch.randn_like(states)*0.1
        # states= states + noises
        ...
        # select actions
        actions, _, _ = self._select_act(states, expert_states_r, soft=False, target=False)
        ...
        if self._exploration_noise is not None:
            if self._active:
                _, uncertainty = self.IL_policy.predict_with_uncertainty(self._state_preprocessor(states))
                # noises = uncertainty*torch.sign(self._exploration_noise.sample(actions.shape))
                noises = uncertainty*self._exploration_noise.sample(actions.shape)*10
                # noises.clamp_(min = -0.3, max = 0.3)False
            else:
                # sample noises
                noises = self._exploration_noise.sample(actions.shape)
    ...
    def _update(self, timestep: int, timesteps: int, pre_train=False, soft = False, dynamics_bc_loss = True, experience_buffer_ratio=0.8, expert_buffer_ratio=0.3, active = True) -> None:
        ...
        #     sampled_states_, sampled_actions_, sampled_rewards_, sampled_next_states_, sampled_dones_ = \
        #         self.memory.sample(names=self._tensors_names, batch_size=1000)[0]
        #     _, uncertainty = self.IL_policy.predict_with_uncertainty(self._state_preprocessor(sampled_states_))
        #     # Find the indices of the top 1000 states with the highest uncertainty
        #     sample_size = 256
        #     uncertainty_np = uncertainty.detach().numpy()
        #     uncertainty_norms = np.linalg.norm(uncertainty_np, axis=1)
        #     # uncertainty_norms = uncertainty_np[:,2]
        #     top_sample_indices = np.argsort(uncertainty_norms)[-sample_size:]
        #     # Sample top states
        #     sampled_states, sampled_actions, sampled_rewards, sampled_next_states, sampled_dones = \
        #         sampled_states_[top_sample_indices], sampled_actions_[top_sample_indices], sampled_rewards_[
        #             top_sample_indices], sampled_next_states_[top_sample_indices], sampled_dones_[top_sample_indices]
        #     expert_states, expert_actions, expert_rewards, expert_next_states, expert_dones = \
        #         self.expert_memory.sample(names=self._tensors_names, batch_size=int(self._batch_size))[0]
        else:
            # sample a batch from memory
            sampled_states, sampled_actions, sampled_rewards, sampled_next_states, sampled_dones = \
            ...
            #     self.memory.sample(names=self._tensors_names, batch_size=int(self._batch_size +1 - self._batch_size * expert_buffer_ratio))[0]
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            with torch.no_grad():
                if pre_train:
                    # target policy smoothing
                    next_actions, _, _ = self.policy.act({"states": expert_next_states}, role="target_policy")
                    ...
                    # select next actions
                    # next_actions, _, _ = self._select_act(sampled_next_states, sampled_next_states, soft=True, target=True)
                    next_actions, _, _ = self._select_act(sampled_next_states, expert_next_states, soft=True, target=True)
                ...
                # target_values = sampled_rewards + 0.998 * sampled_dones.logical_not() * target_qvalues
        ...
        if not self._critic_update_counter % self._policy_delay:
            ...
            if self._active:
                il_actions = self.IL_policy(self._state_preprocessor(sampled_states))
            else:
            ...
            Lq1_loss = F.mse_loss(target_values, avg_critic_values)
            # bc_loss = 0
            ...
            if pre_train:
                ...
                policy_loss = actor_loss + 100* bc_loss + loss_l2
                ...
                if self._BC:
                    actions_bc, _, _ = self.policy.act({"states": expert_states}, role="policy")
                    bc_loss = F.mse_loss(actions_bc, expert_actions)
                    # This BC term is for decreasing extrapolation error
                    alpha = 0.1
                    lambda_ = alpha / avg_critic_values.abs().mean().detach()
                    policy_loss = lambda_*actor_loss+ bc_loss+ loss_l2

                    # actions_bc, _, _ = self.policy.act({"states": sampled_states}, role="policy")
                    # bc_loss = F.mse_loss(actions_bc, sampled_actions)
                    # # policy_loss = lambda_*(-avg_critic_values.mean())+bc_loss
                    # policy_loss = -avg_critic_values.mean()+100*bc_loss
                else:
                    # This BC term is for decreasing extrapolation error
                    alpha = 1
                    lambda_ = alpha / avg_critic_values.abs().mean().detach()
                    actions_bc, _, _ = self.policy.act({"states": sampled_states}, role="policy")
                    bc_loss = F.mse_loss(actions_bc, sampled_actions)
                    # policy_loss = lambda_*(-avg_critic_values.mean())+0*bc_loss
                    policy_loss = -avg_critic_values.mean()+0*bc_loss
            ...
            self.track_data("Target / discount (mean)", torch.mean(discount_values).item())
