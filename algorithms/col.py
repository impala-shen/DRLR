        ...
        for gradient_step in range(self._gradient_steps):
            ...
            if not self._critic_update_counter % self._policy_delay:
                ...
                if pre_train:
                    ...
                    policy_loss = lambda_*(-critic_values.mean())+ F.mse_loss(actions, sampled_actions)
                    ...
                    scaled_q_value = lambda_*(-critic_values.mean())
                    # bc_loss = F.mse_loss(actions, sampled_actions)
                    # policy_loss = lambda_*(-critic_values.mean())+ 1*F.mse_loss(actions, sampled_actions)

                    actions_bc, _, _ = self.policy.act({"states": expert_states_r}, role="policy")
                    bc_loss = F.mse_loss(actions_bc, expert_actions_r)
                    policy_loss = lambda_*(-critic_values.mean()) + 1 * bc_loss
