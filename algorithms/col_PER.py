...
TD3_DEFAULT_CONFIG = {
    ...
    "learning_starts": 5000,           # learning starts after this many steps
...
class Col(Agent):
    ...
    def init(self, trainer_cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.memory is not None:
            ...
            # # # exp_memory = postprocessing.MemoryFileIterator("/home/chen/Downloads/new/memories/cartpole_exp.csv")
        ...
        if self.expert_memory is not None:
            ...
            self._tensors_names = ["states", "actions", "rewards", "next_states", "terminated"]
            # expert_memory = postprocessing.MemoryFileIterator("/home/chen/Downloads/new/memories/ad2.csv")
            expert_memory = postprocessing.MemoryFileIterator(
                "/home/chen/Downloads/new/memories/expert_bc.csv")  ## AEM task
            # expert_memory = postprocessing.MemoryFileIterator("/home/chen/Downloads/new/memories/expert_cab.csv")    ## Others tasks
            # expert_memory = postprocessing.MemoryFileIterator("/home/chen/Downloads/new/memories/cartpole_10gooddemo.csv")    ## Others tasks
            for j, data in expert_memory:
                ...
                for i in range(0, 5000):
                    ...
                    # print(torch.Tensor(np.array(data[keys[4]][i])).logical_not())
            # self.expert_memory.load("/home/chen/Downloads/memories/2000_new.pt")
    ...
    def act(self, states: torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        # if self._random_timesteps < timestep < self._learning_starts:
    ...
    def record_transition(self,
        ...
        if self.memory is not None:
            ...
            current_q1_values, _, _ = self.target_critic_1.act(
                {"states": states, "taken_actions": actions}, role="target_critic_1")
            current_q2_values, _, _ = self.target_critic_2.act(
                {"states": states, "taken_actions":actions}, role="target_critic_2")

            # target policy smoothing
            next_actions, _, _ = self.target_policy.act({"states": next_states}, role="target_policy")
            noises = torch.clamp(self._smooth_regularization_noise.sample(actions.shape),
            ...
            target_q1_values, _, _ = self.target_critic_1.act(
                {"states": next_states, "taken_actions": next_actions}, role="target_critic_1")
            target_q2_values, _, _ = self.target_critic_2.act(
                {"states": next_states, "taken_actions": next_actions}, role="target_critic_2")
            target_q_values = torch.min(target_q1_values, target_q2_values)
            target_values = rewards + self._discount_factor * terminated.logical_not() * target_q_values

            # Compute TD errors for priority updates
            td_error1 = torch.abs(current_q1_values - target_values)
            td_error2 = torch.abs(current_q2_values - target_values)
            errors = torch.max(td_error1, td_error2).detach()

            # storage transition in memory
            self.memory.add_samples(errors = errors, states=states, actions=actions, rewards=rewards, next_states=next_states,
    ...
    def _update(self, timestep: int, timesteps: int, pre_train=False, experience_buffer_ratio=0.8, expert_buffer_ratio=0.2,
                LQ1Gain=1, LBCGain=0.5, LAGain=1) -> None:
        ...
        if pre_train:
            ...
            sampled_states, sampled_actions, sampled_rewards, sampled_next_states, sampled_dones, importance_weights = \
                self.memory.sample(names=self._tensors_names, batch_size=self._batch_size)[0]
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            with torch.no_grad():
                ...
                # compute current values
                current_q1_values, _, _ = self.target_critic_1.act(
                    {"states": sampled_states, "taken_actions": sampled_actions}, role="target_critic_1")
                current_q2_values, _, _ = self.target_critic_2.act(
                    {"states": sampled_states, "taken_actions": sampled_actions}, role="target_critic_2")
                ...
                td_error1 = torch.abs(current_q1_values - target_values)
                td_error2 = torch.abs(current_q2_values - target_values)
                td_errors = torch.max(td_error1, td_error2).detach()


            # Get current Q estimates, and compute critic loss
            # compute critic loss
            ...
            if pre_train:
                ...
                elementwise_loss_1 = F.mse_loss(critic_1_values, target_values, reduction='none')
                elementwise_loss_2 = F.mse_loss(critic_2_values, target_values, reduction='none')

                # Weight the losses with importance sampling weights
                weighted_loss_1 = (elementwise_loss_1 * importance_weights).mean()
                weighted_loss_2 = (elementwise_loss_2 * importance_weights).mean()

                critic_loss = weighted_loss_1 + weighted_loss_2
            ...
            if not self._critic_update_counter % self._policy_delay:
                ...
                # if pre_train:
                # critic_values, _, _ = self.critic_1.act({"states": sampled_states, "taken_actions": sampled_actions}, role="critic_1")
                # else:
            ...
            if not self._critic_update_counter % self._policy_delay:
                ...
                self.track_data("Loss / Lq1 loss", Lq1_loss.item())
        ...
        # Update priorities in the PER memory after all gradient steps
