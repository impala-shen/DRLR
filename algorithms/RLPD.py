from typing import Any, Mapping, Dict, Optional, Tuple, Union
...
class SAC(Agent):
    ...
    def _select_act(self, obs: torch.Tensor,
        ...
        if target:
            ...
            rl_actions, next_log_prob, _ = self.policy.act({"states": obs}, role="policy")
            return rl_actions, next_log_prob, _
        else:
            # sample stochastic actions
            with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                rl_actions, _, outputs = self.policy.act({"states": self._state_preprocessor(obs)}, role="policy")
                return rl_actions, _, _
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            if self._offline:
                ...
                sampled_states_r, sampled_actions_r, sampled_rewards_r, sampled_next_states_r, sampled_dones_r = \
                    self.memory.sample(names=self._tensors_names, batch_size=int(self._batch_size * 0.5))[0]
                expert_states_r, expert_actions_r, expert_rewards_r, expert_next_states_r, expert_dones_r = \
                    self.expert_memory.sample(names=self._tensors_names, batch_size=int(self._batch_size - self._batch_size * 0.5))[0]
