...
class IBRL(Agent):
    ...
    def _select_act(self, obs: torch.Tensor,
        ...
        target_q_rl, _, _ = self.critic_1.act(
            {"states": obs, "taken_actions": rl_actions},
            role="critic_1"
        )
        # target_q_il = self._compute_min_q_values(obs, il_actions)
        target_q_il, _, _ = self.critic_2.act(
            {"states": obs, "taken_actions": il_actions},
            role="critic_2"
        )
    ...
    def _update(self, timestep: int, timesteps: int, pre_train=False, soft = False, dynamics_bc_loss = True, experience_buffer_ratio=1, expert_buffer_ratio=1) -> None:
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            with torch.no_grad():
                ...
                target_qvalues, _, _ = self.target_critic_1.act(
                    {"states": sampled_next_states, "taken_actions": next_actions},
                    role="target_critic_1"
                )
                target_qvalues_exp, _, _ = self.target_critic_2.act(
                    {"states": expert_next_states_r, "taken_actions": next_actions},
                    role="target_critic_2"
                )
                ...
                # Compute exp target values
                target_values_exp= expert_rewards_r + self._discount_factor * sampled_dones.logical_not() * target_qvalues_exp
            ...
            critic_2_values, _, _ = self.critic_2.act({"states": expert_states_r, "taken_actions": expert_actions_r}, role="critic_2")

            critic_loss = F.mse_loss(critic_1_values, target_values)
            critic_loss_exp = F.mse_loss(target_values_exp, critic_2_values)

            # optimization step (critic)
            self.critic_optimizer.zero_grad()
            critic_loss.backward()
            critic_loss_exp.backward()
            ...
            self.track_data("Target / exp"
                            " (mean)", torch.mean(target_qvalues_exp).item())
