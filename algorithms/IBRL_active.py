...
class IBRL(Agent):
    ...
    def _select_act(self, obs: torch.Tensor,
        ...
        if target:
            ...
            rl_actions, _, _ = self.target_policy.act({"states": self._state_preprocessor(obs)}, role="target_policy")
        ...
        # Get IL actions
        self.IL_policy.eval()
        # il_actions, _ = self.IL_policy.predict_with_uncertainty(self._state_preprocessor(obs))
        il_actions = self.IL_policy(self._state_preprocessor(self._state_preprocessor(obs)))[0]
        # print(actions)
    ...
    def act(self, states: torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        if timestep < self._random_timesteps:
            self.IL_policy.eval()
            # il_actions, _, _ = self.IL_policy.act({"states": self._state_preprocessor(states)}, role="policy")
            # actions, _ = self.IL_policy.predict_with_uncertainty(self._state_preprocessor(states))
            actions = self.IL_policy(self._state_preprocessor(states))[0]
            return actions

        # select actions
        actions, _, _ = self._select_act(states, soft=True, target=False)
    ...
    def _update(self, timestep: int, timesteps: int, pre_train=False, soft = False, experience_buffer_ratio=0.5, expert_buffer_ratio=0.5, active = True) -> None:
        ...
        if pre_train:
            ...
            # sample noises
            noises = torch.randn_like(sampled_states) * 0.1
            ...
            # sampled_states = torch.cat((sampled_states_r, expert_states))
            # sampled_actions = torch.cat((sampled_actions_r, expert_actions))
            # sampled_next_states = torch.cat((sampled_next_states_r, expert_next_states))
            # sampled_rewards = torch.cat((sampled_rewards_r, expert_rewards))
            # sampled_dones = torch.cat((sampled_dones_r, expert_dones))
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            with torch.no_grad():
                # select actions
                next_actions, _, _ = self._select_act(sampled_next_states, soft=True, target=True)
            ...
            if not self._critic_update_counter % self._policy_delay:
                ...
                if pre_train:
                    ...
                    policy_loss = bc_loss + policy_loss
                else:
